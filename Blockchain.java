...
public class Blockchain {
	...
	public boolean isValidNewBlock(Block newBlock, Block previousBlock) {
		if (newBlock != null && previousBlock != null) {
			//check that the two blocks are in sequence
			if (previousBlock.getIndex() + 1 != newBlock.getIndex()) {
				return false;
			}
			
			//compare previous hash in the new block to the hash of the previous block 
			if (newBlock.getPreviousHash() == null || !newBlock.getPreviousHash().equals(previousBlock.getHash())) {
				return false;
			}
			
			//compare calculated hash of the new block equals its registered hash
	...
	public boolean isBlockChainValid() {
		...
		//loop through blockchain to check all hashes
